...
"""
To do:
    • Simplify layout - single subplot. Remove "Trace"
    • Assume simple input of 'time' and 'signal' in df
    • Assume window bounds and sample_name is passed by user
        • e.g., window_bounds[xmin, xmax]
    • Reference - look in hplc_integration.py for call
"""
class SignalAnalyzer:
    def __init__(self, df, window_bounds, gaus_iterations, sample_name, max_peaks, sw, sf, pk_sns, pk_pr, max_PA, is_reference=True, reference_peaks=None):
    ...
    def setup_subplot(self, ax):
        ...
        x_values = self.df["RT (min)"] #Used to be "rt_corr"
        y = self.df["Signal"]
